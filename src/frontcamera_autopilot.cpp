  ...
  while ( nh.ok() )
    {
      ...
      // Modify current_offset in proximity of the target
			cout << current_offset.GetRoll() << endl;
                  ...
                  if ( count == 1 )
                  ...
                  else
                    {
                      ...
                      // Update the compensatory factor for drift on roll
											cout << "compensatory_offset: " << compensatory_offset.GetRoll() << ", current_offset: " << current_offset.GetRoll() <<", last_offset: " << last_view_offset.GetRoll() << endl;
                      k_roll = GetCompensatoryFactor ( compensatory_offset.GetRoll(), last_view_offset.GetRoll(), count, x_axis_crossed, target_x );
