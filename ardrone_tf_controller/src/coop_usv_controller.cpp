...
string MarkerLost(int *lost_count, int *count, int k_roll, int k_pitch, int k_gaz, bool *was_reverse, bool *initialization_after_tf_lost, bool *tf_lost_compensatory, int *multiplier, Offset *last_view_offset, bool *critical_phase, float target_x, float target_y, float target_z, bool *marker_was_lost, int branch, int axis_crossed_mul);
void isAxisCrossed(double last_offset, double current_offset, int count, bool *axis_crossed);
...
int main ( int argc, char **argv )
{
  ...
  ros::Subscriber ekf_sub = nh_husky.subscribe ( "/usv/p3d_odom",1, ekfCallback);
  ...
  ros::Publisher stop_husky_pub = nh.advertise<geometry_msgs::Twist> ("/usv/cmd_vel",100);
  ros::Publisher reset_filter_pub = nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("/usv/set_pose", 1000);
  ...
  while ( nh.ok() )
    {
      ...
      if ( marker_detected == true )
        {
          ...
          //std::cout << "Yaw: " << std::abs ( yaw ) << std::endl;
          if ( ( std::abs ( yaw ) < std::abs ( epsilon ) && front_camera == true ) || ( ( std::abs ( yaw ) < ( PI/2 ) ) && front_camera == false ) )
            {
      ...
      else tf_lost_compensatory = false;

      //std::cout << "Value of tf_lost_compensatory: " << tf_lost_compensatory << std::endl;
      ...
      if(std::abs(current_offset.GetRoll()) < 0.5 && std::abs(current_offset.GetPitch()) < 0.5 )
        {
      ...
      else
        {
          ...
          if ( current_time_sec <= ( last_msg + 2 ) )
          ...
          else
            {
              ...
              // Check if the y axis has been crossed
              std::cout<< "Value of y_axis_crossed: " << y_axis_crossed << std::endl;
              isAxisCrossed(last_view_offset.GetPitch(), compensatory_offset.GetPitch(), count, &y_axis_crossed);
              std::cout<< "New value of y_axis_crossed: " << y_axis_crossed << std::endl;

              // y_axis_crossed = true when the UAV is behind the y-axis
              if (y_axis_crossed)
                {
                  axis_crossed_mul = -1;
                }
              else axis_crossed_mul = 1;

              move_by_rel.data = MarkerLost(&lost_count, &count, k_roll, k_pitch, k_gaz, &was_reverse, &initialization_after_tf_lost, &tf_lost_compensatory, &multiplier, &compensatory_offset, &critical_phase, target_x, target_y, target_z, &marker_was_lost, branch, axis_crossed_mul);
              cmd_pub.publish<> ( move_by_rel );
...
float getCompensatoryFactor ( double current_offset, double last_offset, int count, bool &axis_crossed, double target, int type )
{
  ...
  isAxisCrossed(last_offset, current_offset, count, &axis_crossed);
...
string MarkerLost(int *lost_count, int *count, int k_roll, int k_pitch, int k_gaz, bool *was_reverse, bool *initialization_after_tf_lost, bool *tf_lost_compensatory, int *multiplier, Offset *last_view_offset, bool *critical_phase, float target_x, float target_y, float target_z, bool *marker_was_lost, int branch, int axis_crossed_mul)
{
  ...
  //std::cout << "Lost_count : " << *lost_count << endl;

  if ( *lost_count == 0)
    {
      ROS_WARN("STOP UAV WHEN MARKER IS LOST");
      control_cmd = "c moveByRel 0 0 0 0";
    }
  else if ( *count > 1 )
    {
      ...
      if ( *was_reverse == true || *initialization_after_tf_lost == false )
        {
          if (*tf_lost_compensatory == true)
            {
              ROS_WARN(" CRITICAL PHASE 1 - change multiplier !!");
              *multiplier = -1;
              *critical_phase = true;
            }
      ...
      /*
      else if ( *initialization_after_tf_lost == false)
        {
          ROS_WARN(" CRITICAL PHASE 3 - change multiplier !!");
          *critical_phase = false;
          *multiplier = -1;
        }*/
      else
        {
          ROS_WARN(" CRITICAL PHASE 2 - do not change multiplier !!");
        }
      //cout << "Multiplier: " << *multiplier << endl;

      //if(branch == 1) branch_mul = 1;
      //else branch_mul = -1;
      //std::cout << "Branch_mul = " << branch_mul << std::endl;

      // do not rotate while the marker is lost
      ...
      control_cmd = "c moveByRel " + boost::lexical_cast<std::string> ( (*multiplier) * k_roll * last_view_offset->GetRoll() /* +  target_x * (*lost_count)*/) + " "+
          boost::lexical_cast<std::string> ( axis_crossed_mul * std::abs( (*multiplier) * k_pitch * last_view_offset->GetPitch()) /* + target_y * (*lost_count)*/) + " " +
          boost::lexical_cast<std::string> ( k_gaz * 0.5  * (*lost_count) ) + " " +
...
void isAxisCrossed(double last_offset, double current_offset, int count, bool *axis_crossed)
{
  // if the UAV cross the axis, change the value
  if ( last_offset * current_offset <= 0 && count > 2 )
    {
      ROS_ERROR("Y AXIS CROSSED!");
      //if ( *axis_crossed == false )
      //  *axis_crossed = true;
      !(*axis_crossed);
    }
  // if the UAV is on one side of the axis, keep the value constant
  else if ( last_offset * current_offset >= 0 && *axis_crossed == true )
    {
      // do nothing
    }
}
